...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    icon: <DollarLineIcon />,
    subItems: [
      { name: "Assinaturas", path: "/financeiro/assinaturas", pro: false },
      { name: "Pagamentos", path: "/financeiro/pagamentos", pro: false },
    ],
  },
];
...
const AppSidebar: React.FC = () => {
        ...
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            >
              ...
              {(isExpanded || isHovered || isMobileOpen) && (
                <ChevronDownIcon
                  className={`ml-auto flex-shrink-0 w-5 h-5 transition-transform duration-200 ${
  ...
  // Check if a path is active - considers sub-routes as active
  const isActive = useCallback((path: string) => {
    // Exact match
    if (path === pathname) return true;

    // For non-root paths, check if pathname starts with the path
    // This makes sub-routes (like /planejamento/instituicoes/criar)
    // also activate the parent menu item (/planejamento/instituicoes)
